...
- ✅3 ProcessingFinished 死信号：发送方与协调器变体全删（或启用）
- ✅4 StartResult 回滚接上：效果线程执行 Start 后回发真实结果，失败复位协调器（麦克风被拒场景）

## P2 缺机制
- ✅5 引擎结算 settle 在 doubao/openai_realtime 重复 → 公共 settle（契约只写一次）
- ✅6 收尾超时各自硬编码（doubao 3s / openai 1800ms）→ 统一常量
- ✅7 麦克风解析无缓存：每次按下全枚举（+40~110ms 按键延迟）→ 缓存上次 uid，miss 才枚举
- ✅8 ctrl_stop 同步写盘阻塞效果线程 → save_wav/prune 挪交付线程（pcm 随 handoff 流动）
...
- ✅17 restore_clipboard 双默认相反（serde default=false / 读取 fallback=true）→ 统一 default_true
- ✅18 settings 死字段群删除（shortcut_activation/hold_threshold_ms/filler_word_removal/append_trailing_space 等无读者字段）
