...
> 教训来源：C27（物理/逻辑坐标混算）、C41（NS-y-up 与 CG-y-down 混用）、C26/C39（主线程 API 在回调里直调）。
...
规则：一次计算只用一套体系，从头到尾。当前坐标操作**收口在 overlay.rs**（全 NS 链路）——新增坐标代码要么进 overlay.rs，要么注明所用体系。
...
- 全局快捷键回调：主线程，**只发信号**（C26），禁止内部再碰需主线程同步的 API
- tokio：引擎会话跑专用 current_thread runtime（C35），勿挂 tauri::async_runtime

## 生命周期
- objc2 的 Retained 自动释放；裸 CGEvent/CF 类型必须 CFRelease
- 录音会话附属物（定时器/循环/桥线程）一律持 `Recording.cancel` 令牌，会话 Drop 自知作废
