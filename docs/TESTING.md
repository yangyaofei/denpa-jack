...
每修一个 bug 必加对应用例（Handy settings.rs 内联 JSON fixture / history.rs 内存库同款手法）。
...
- 我们差距补齐: tempfile 手法✅/前端冒烟✅/链路回归✅(regression.sh, Handy 无此层——我们的 autotest 通道更强)/tokio::test 层暂缺(当前异步面窄, 需要时补)
