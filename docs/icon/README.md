...
- `bake_app_icon.py`：`qlmanage` 渲染 `icon.svg` → 去掉白底得字形 alpha → 与卡片合成 → 导出 1024 图与状态栏图。
...
| 中心线 + 傅里叶 / 样条 | 最终路线：算法提取中心线 + 样条平滑（`build5.py`） |
| 卡片配色 | 近白卡片在 macOS 26 系统底板上只剩一圈灰边 → 逐一试过蓝/粉/纯黑卡片，最终定为**白卡片 + 纯黑字形** |
| 卡片结构 | 卡片占画布 80.4%、圆角半径 = 卡片边长的 25%、纯色、轻阴影（对齐 Handy 的打包结构） |
