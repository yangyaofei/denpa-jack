...
上游 0.3.4 有两个 macOS 键盘状态缺陷会导致本项目“修饰键卡在按下状态 / 纯修饰热键无法触发”，
...
- 用 `rev` 而非 `branch`：分支后续变动不影响构建；`src-tauri/Cargo.lock` 也锁定同一版本。
...
- **问题**：上游只在**非 FlagsChanged** 事件里调用 `reconcile_modifiers()` 对账。系统在 tap 被禁用
  ...
  （`Modifiers::matches`），多余的位会让 `Fn` 单独不再匹配 → 热键失效，直到用户按下几次其他普通键。
...
- **已知限制**：CGEventFlags 不区分左右。同组（如左右 Cmd）两键同时按住时松开一侧，该侧的位会
  保留到整组释放。组匹配（`Modifiers::matches` 按 compound 组任意一侧）不受影响，下一次对账清除。
...
- PR：https://github.com/handy-computer/handy-keys/pull/37
  （分支 `fix/missed-modifier-events-reconcile`；改动仅 `src/platform/macos/listener.rs`，25+/29−；
   `cargo check` 无警告、`cargo test` 6 项通过）
- **上游合入并发布修复版后**：
  1. 删除 `Cargo.toml` 的 `[patch.crates-io]` 段；
  2. 依赖改为 crates.io 的修复版（如 `handy-keys = "0.4"`）；
  3. 更新 `Cargo.lock`；4. 更新本文件。
- **需要再改补丁时**：在 fork 分支提交 → 更新这里的 `rev` → `cargo update -p handy-keys` → 运行 `cargo test --lib`。

## 验证方式

- 复现：按住 Cmd → 粘贴到非激活窗口（Cmd+V）→ 按纯修饰热键（如 Fn）→ 修复前失效，按下几次其他键才恢复。
- 验证：用事件探针打印原始 CGEvent 字段，粘贴后立即按 Fn 连续 5 次全部正确；80ms 间隔连按 15 次
  得到 15 组完整 down/up。本项目 `cargo test --lib` 76 项全部通过。
