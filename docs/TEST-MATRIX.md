...
| recording | 6 | **C40 双输入** wav_data_chunk 跳 FLLR 填充/垃圾拒绝 / **C8** 热词 100token 预算 / resolve_asr 未知引擎拒绝+首档回退+空 Key 兑池 |
| settings | 4 | normalize_key/shortcut_str/**C17 缺字段不清空**（顺带修出 serde 默认值 0 的隐患）/配置往返 |
| llm | 4 | tool_calls 揔回/降级 content/空 None/坏 arguments 降级 |
