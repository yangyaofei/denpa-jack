...
10. **修 bug 先查历史行为（回归优先）**：动手前先用 `git log -S<符号>` / `git diff <已知正常版本>..HEAD -- <文件>`
    确认"这个行为以前有没有"。以前**没有** → 是最近改动引入的回归，去改引入它的那段逻辑、回到原来的做法；
    以前**有** → 才是新问题，单独修。禁止在原逻辑上再叠一层新实现绕过（一层问题叠成两套并存逻辑，
    现象不再能唯一归因，下次更难查）。
